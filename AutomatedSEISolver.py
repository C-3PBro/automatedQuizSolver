...
if __name__ == '__main__':
    ...
    with plt.style.context(('seaborn')):
        plt.hist(solutions, bins=len(np.unique(solutions)), edgecolor='black',linewidth=1.2)
        plt.xticks(rotation=45)
        plt.title(r'\textbf{Frequency of different solutions}'
                  + '\nTotal trials: ' + str(len(solutions))
                  + '\nNumber of different solutions: ' + str(len(np.unique(solutions))))
    solutions2DataFrame = pd.DataFrame(solutions)
    # Save data
    fileList = glob.glob('*.csv')
    if fileList.count('test.csv'):
        dataFrame = pd.read_pickle('test.csv')
        dataFrame.append(solutions2DataFrame, ignore_index=True)
        dataFrame.to_pickle('test.csv')
    else:
        solutions2DataFrame.to_pickle('test.csv')
